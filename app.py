...
app = FastAPI(title="数据库面板")

# ========== CockroachDB ==========
def query_cockroachdb(sql):
    conn = psycopg2.connect(
        host='dyed-muskox-27479.j77.aws-ap-southeast-1.cockroachlabs.cloud',
        port=26257,
        user='zhangbowen',
        password='9CNaITmol_9RWsG2TaNVDw',
        dbname='defaultdb',
        sslmode='require'
    )
    cur = conn.cursor()
...
# ========== Turso ==========
with open(os.path.join(os.path.dirname(__file__), "turso_token.txt"), "r") as _f:
    TURSO_TOKEN = _f.read().strip()
TURSO_URL = "https://zhangbowenmain-zhangbowen.aws-ap-south-1.turso.io/v2/pipeline"
...
async function switchDb(db) {
  ...
  if (db === 'cockroachdb') {
    document.getElementById('db-name').textContent = 'CockroachDB';
    document.getElementById('db-type').textContent = 'PostgreSQL 兼容';
    document.getElementById('db-region').textContent = 'AWS Singapore';
  } else {
    document.getElementById('db-name').textContent = 'Turso';
    document.getElementById('db-type').textContent = 'SQLite 边缘分布式';
    document.getElementById('db-region').textContent = 'AWS Mumbai';
  }
  await loadTables();
}
...
async function loadTables() {
  ...
  try {
    const resp = await fetch(`/api/${currentDb}/tables`);
    const data = await resp.json();
    tables[currentDb] = data.tables;
    select.innerHTML = data.tables.map(t => `<option value="${t}">${t}</option>`).join('');
    if (data.tables.length > 0) loadTable();
  } catch (e) {
...
async function loadTable() {
  ...
  try {
    ...
    if (data.rows.length === 0) {
      document.getElementById('table-content').innerHTML = '<div class="loading">表为空</div>';
      return;
    }
